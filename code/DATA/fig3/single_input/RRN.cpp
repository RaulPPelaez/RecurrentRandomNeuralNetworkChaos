...
public:
  ...
  void save(){
    ...
    fori(0,N){
      forj(0,N) out<<setprecision(6)<<Wrec[i][j]<<" ";
      out<<"\n";
    }
    
    fori(0,N) out<<setprecision(6)<<Wout[i]<<"\n";
    fori(0,N) out<<setprecision(6)<<Win[i]<<"\n";
    fori(0,target[0].size()) out<<setprecision(6)<<target[0][i]<<"\n";
  }
...
int main(int argc, char* argv[]){
  ...
  sp.nsteps = int((sp.end_train+500)/sp.dt);
  sp.nloops    = 1;
